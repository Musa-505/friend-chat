...
Жаңа нұсқа GitHub-қа пушталғанда, барлық клиенттер өзін-өзі жаңартады:

- **friend.py** — әр командада GitHub-тан жаңа нұсқаны тексереді (сағатына бір рет), жаңасы болса өзін + daemon.py-ді жаңартады
- **daemon.py** — іске қосылғанда тексереді
- **VM сервері** — cron арқылы әр 5 минут сайын GitHub-тан тартып, сервисті қайта іске қосады

Қолмен тексеру: `friend.py update`

### Жаңа нұсқа шығару

1. `friend.py` және `daemon.py` ішінде `VERSION` мәнін көтеріңіз (мысалы `1.1.0` → `1.2.0`)
2. GitHub-қа пуштаңыз
3. Клиенттер келесі тексеруде автоматты жаңарады
